...
//no clue if this works on msvc
//nor do I have any clue if snprintf exists either
//#ifndef HAVE_SSIZE_T
//#if defined(_WIN64)
...
extern netplay_t * zmz_netplay_handle;
...
{
   zmz_show_message("Netplay has disconnected. Will continue without connection.");
}

void input_poll_net(void)
{
   if (!netplay_should_skip(zmz_netplay_handle) && netplay_can_poll(zmz_netplay_handle))
      netplay_poll(zmz_netplay_handle);
}

void video_frame_net(const void *data, unsigned width, unsigned height, size_t pitch)
{
   if (!netplay_should_skip(zmz_netplay_handle))
      zmz_netplay_handle->cbs.frame_cb(data, width, height, pitch);
}

void audio_sample_net(int16_t left, int16_t right)
{
   if (!netplay_should_skip(zmz_netplay_handle))
      zmz_netplay_handle->cbs.sample_cb(left, right);
}

size_t audio_sample_batch_net(const int16_t *data, size_t frames)
{
   if (!netplay_should_skip(zmz_netplay_handle))
      return zmz_netplay_handle->cbs.sample_batch_cb(data, frames);
   else
      return frames;
}
...
{
   if (netplay_is_alive(zmz_netplay_handle))
      return netplay_input_state(zmz_netplay_handle, port, device, index, id);
   else
      return zmz_netplay_handle->cbs.state_cb(port, device, index, id);
}
...
static uint32_t implementation_magic_value(void)
{
   ...
   const char *lib = info.library_name;
...
static bool send_info(netplay_t *handle)
{
   uint32_t header[3] = {
      ...
      htonl(retro_get_memory_size(RETRO_MEMORY_SAVE_RAM))
   };
   ...
   unsigned sram_size = retro_get_memory_size(RETRO_MEMORY_SAVE_RAM);
...
static bool get_info(netplay_t *handle)
{
   ...
   if (retro_get_memory_size(RETRO_MEMORY_SAVE_RAM) != ntohl(header[2]))
   {
   ...
   const void *sram = retro_get_memory_data(RETRO_MEMORY_SAVE_RAM);
   unsigned sram_size = retro_get_memory_size(RETRO_MEMORY_SAVE_RAM);
...
static uint32_t *bsv_header_generate(size_t *size, uint32_t magic)
{
   ...
   *size = header_size;
   ...
   bsv_header[CRC_INDEX] = swap_if_big32(CRC32);
   bsv_header[STATE_SIZE_INDEX] = swap_if_big32(serialize_size);

   if (serialize_size && !retro_serialize(header + 4, serialize_size))
   {
      free(header);
      return NULL;
   }
...
static bool bsv_parse_header(const uint32_t *header, uint32_t magic)
{
   ...
   if (in_state_size != retro_serialize_size())
   {
      RARCH_ERR("Serialization size mismatch, got 0x%x, expected 0x%x.\n",
            (unsigned)in_state_size, (unsigned)retro_serialize_size());
      return false;
   }
...
static bool get_info_spectate(netplay_t *handle)
{
   ...
   size_t save_state_size = retro_serialize_size();
   ...
   bool ret = true;
   if (save_state_size)
      ret = retro_unserialize(buf, save_state_size);
...
static void init_buffers(netplay_t *handle)
{
   ...
   handle->state_size = retro_serialize_size();
   unsigned i;
...
static bool netplay_get_cmd(netplay_t *handle)
{
   ...
   switch (cmd)
   {
      case NETPLAY_CMD_FLIP_PLAYERS:
      {
         ...
         zmz_show_message("Netplay players are flipped.");
...
{
   retro_serialize(handle->buffer[handle->self_ptr].state, handle->state_size);
...
{
   int16_t res = zmz_netplay_handle->cbs.state_cb(port, device, index, id);
   netplay_set_spectate_input(zmz_netplay_handle, res);
   return res;
}
...
static int16_t netplay_get_spectate_input(netplay_t *handle, bool port, unsigned device, unsigned index, unsigned id)
{
   ...
   else
   {
      ...
      zmz_show_message("Connection with host was cut.");

      retro_set_input_state(zmz_netplay_handle->cbs.state_cb);
      return zmz_netplay_handle->cbs.state_cb(port, device, index, id);
   }
}
...
{
   return netplay_get_spectate_input(zmz_netplay_handle, port, device, index, id);
}
...
static void netplay_post_frame_net(netplay_t *handle)
{
   ...
   if (handle->other_frame_count < handle->read_frame_count)
   {
      ...
      retro_unserialize(handle->buffer[handle->other_ptr].state, handle->state_size);
      bool first = true;
      while (first || (handle->tmp_ptr != handle->self_ptr))
      {
         retro_serialize(handle->buffer[handle->tmp_ptr].state, handle->state_size);
