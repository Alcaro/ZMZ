...
static void allocmem()
{
  ...
  newgfx16b = 1;
  int sizes[]={64*0x100000, 32*0x100000, 16*0x100000, 8*0x100000, 6*0x100000, 4*0x100000, 2*0x100000, 0};
  int sizeid;
  for (sizeid=0;true;sizeid++)
  {
    if (!sizes[sizeid]) outofmemory();
    if ((romaptr = malloc(sizes[sizeid]+32768*2+4096)))
    {
      maxromspace = sizes[sizeid];
      break;
    }
  }
