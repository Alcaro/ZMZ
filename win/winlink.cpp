...
BOOL InputAcquire(void)
{
   ...
   if (device1 && device2 && !GUIOn2) MultiMouseInit();
      else if (MouseInput && GUIOn2 && FullScreen) MouseInput->Acquire();
...
BOOL InputReAcquire(void)
{
   if (InputEn)
   {
      if (MouseInput && GUIOn2 && FullScreen) MouseInput->Acquire();
      else MouseInput->Unacquire();
   }
   return TRUE;
}
...
{
	if (visible!=cursorVisible) ShowCursor(visible);
	cursorVisible=visible;
}

BOOL InputRead(void)
{
   if (MouseInput && GUIOn2 && !FullScreen)
   {
      static POINT PrevMousePos;
      POINT ThisMousePos;
      GetCursorPos(&ThisMousePos);
      MouseMoveX=(float)ThisMousePos.x-PrevMousePos.x;
      MouseMoveY=(float)ThisMousePos.y-PrevMousePos.y;
      PrevMousePos=ThisMousePos;
      
      RECT windowRect;
      GetWindowRect(hMainWindow, &windowRect);
      bool mouseInWindow=PtInRect(&windowRect, ThisMousePos);
      setCursor(!mouseInWindow);
      if (MouseButton || mouseInWindow)
      {
         MouseButton=GetAsyncKeyState(lhguimouse?VK_RBUTTON:VK_LBUTTON)?1:0;
         MouseButton|=GetAsyncKeyState(lhguimouse?VK_LBUTTON:VK_RBUTTON)?2:0;
      }
      else
      {
         MouseButton=0;
...
LRESULT CALLBACK Main_Proc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
   ...
   switch (uMsg)
   {
      ...
      case WM_MOUSEMOVE:
         ...
         {
            if (FullScreen) MouseInput->Acquire();
            else
            {
               MouseX=GET_X_LPARAM(lParam)*256.0/WindowWidth;
               MouseY=GET_Y_LPARAM(lParam)*224.0/WindowHeight;
               MouseButtonPressed = 0;
               if (!lhguimouse && (wParam&MK_LBUTTON)) MouseButtonPressed = 1;
               if (lhguimouse && (wParam&MK_RBUTTON)) MouseButtonPressed = 1;
               MouseButton=(wParam&(MK_LBUTTON|MK_RBUTTON));
            }
         }
         break;
      case WM_MOUSEWHEEL:
         {
            long zDelta = ((signed short)HIWORD(wParam))/3;
            while (zDelta>0){
              zDelta-=40;
              if (!((CurKeyPos+1==CurKeyReadPos) || ((CurKeyPos+1==16)
                 && (CurKeyReadPos==0)))){
...
DWORD SampleRateActual;

extern "C" { static void ClearSound(); }

static BOOL InitSoundShared()
{
   WAVEFORMATEX wfx;
   DSBCAPS dsbcaps;
   ...
   SoundBufferSize = SampleRate;
   while (SoundBufferSize & (SoundBufferSize-1)) SoundBufferSize&=(SoundBufferSize-1);
   SoundBufferSize*=2;
...
//FullScreen=0;
   ...
   if (((PrevStereoSound!=StereoSound)||(PrevSampleRate!=SampleRate)||(PrevSoundQuality!=SoundQuality))&&FirstSound!=1)
      ReInitSound();
...
static void ClearSound()
{
	...
	if(!UsePrimaryBuffer)
	{
		if (DS_OK!=lpSoundBuffer->Lock(0, 0, &lpvPtr1, &dwBytes1, &lpvPtr2, &dwBytes2, DSBLOCK_ENTIREBUFFER))
		{
			return;
		}
	}
	...
	{
		if (DS_OK!=lpPrimaryBuffer->Lock(0, 0, &lpvPtr1, &dwBytes1, &lpvPtr2, &dwBytes2, DSBLOCK_ENTIREBUFFER))
		{
			return;
		}
	}
	...
	if(!UsePrimaryBuffer)
	{
		...
		lpSoundBuffer->GetCurrentPosition(NULL, &writecursor);
	}
	else
	{
		...
		lpPrimaryBuffer->GetCurrentPosition(NULL, &writecursor);
	}
}

extern unsigned char MusicVol;

static void UpdateSoundInternal(const short * data, size_t frames)
{
	LPVOID lpvPtr1;
	LPVOID lpvPtr2;
	DWORD dwBytes1;
	...
	{
		if (DS_OK!=lpPrimaryBuffer->Lock(writecursor, frames*4, &lpvPtr1, &dwBytes1, &lpvPtr2, &dwBytes2, 0))
		{
			return;
		}
	}
	...
	unsigned int nsamples1=dwBytes1/2;
	unsigned int nsamples2=dwBytes2/2;
	
	for (unsigned int i=0;i<nsamples1;i++)
	{
		samples1[i]=data[i]*MusicVol/127;
	}
	for (unsigned int i=0;i<nsamples2;i++)
	{
		samples2[i]=data[nsamples1+i]*MusicVol/127;
	}
...
void UpdateSound(const short * data, size_t frames)
{
	...
	if(!UsePrimaryBuffer) lpSoundBuffer->GetCurrentPosition(&writecursortrue, NULL);
	else lpPrimaryBuffer->GetCurrentPosition(&writecursortrue, NULL);
	int difference=((writecursor+SoundBufferSize-writecursortrue)%SoundBufferSize);
	//if (!UsePrimaryBuffer)
	//{
		//SampleRateActual=SampleRate + difference*1500/SoundBufferSize - 750;
		//lpSoundBuffer->SetFrequency(SampleRateActual);
	//}
...
void WinUpdateDevices()
{
   ...
   if ((keys2[0xB8] != 0 || keys2[0x38] != 0) && keys2[0x1C] != 0)
   {
...
{
   InputRead();
   if (FullScreen!=1) return((int) MouseX);
...
int GetMouseY(void)
{
   if (FullScreen!=1) return((int) MouseY);
...
int GetMouseButton(void)
{
   ...
   if (MouseButton&(lhguimouse?1:2) && GetForegroundWindow()==hMainWindow)
   {
